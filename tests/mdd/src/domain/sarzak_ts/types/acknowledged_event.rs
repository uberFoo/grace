...
impl AcknowledgedEvent {
    ...
    pub fn new(event_id: &Event, state_id: &State, store: &mut SarzakTsStore) -> AcknowledgedEvent {
        let id = Uuid::new_v4();
    ...
    pub fn r20_event<'a>(&'a self, store: &'a SarzakTsStore) -> Vec<&Event> {
    ...
    pub fn r20_state<'a>(&'a self, store: &'a SarzakTsStore) -> Vec<&State> {
