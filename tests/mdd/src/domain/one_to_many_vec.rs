...
mod tests {
    ...
    fn test_r1() {
        ...
        let tgt = a_0.borrow().r1_referent(&store);
        assert!(tgt.len() == 1);
        assert_eq!(tgt_0, tgt[0]);

        let tgt = a_1.borrow().r1_referent(&store);
        assert!(tgt.len() == 1);
        assert_eq!(tgt_1, tgt[0]);

        let tgt = a_2.borrow().r1_referent(&store);
        assert!(tgt.len() == 1);
        assert_eq!(tgt_1, tgt[0]);

        // Test in the many direction
        let a = tgt_0.borrow().r1_a(&store);
        assert_eq!(1, a.len());
        assert_eq!(a_0, a[0]);

        let a = tgt_1.borrow().r1_a(&store);
        assert_eq!(2, a.len());

        // Result contains a_1 and a_2, but in any order.
        let an_a = a.iter().find(|a| a.borrow().id == a_1.borrow().id);
        assert!(an_a.is_some());
        assert_eq!(Some(&a_1), an_a);

        let an_a = a.iter().find(|a| a.borrow().id == a_2.borrow().id);
        assert!(an_a.is_some());
    ...
    fn test_r2() {
        ...
        let tgt = b_2.borrow().r2_referent(&store);
        assert!(tgt.len() == 1);
        assert_eq!(tgt_1, tgt[0]);
        ...
        let a_b = b.iter().find(|b| b.borrow().id == b_1.borrow().id);
        assert!(a_b.is_some());
        assert_eq!(Some(&b_1), a_b);

        let a_b = b.iter().find(|b| b.borrow().id == b_2.borrow().id);
        assert!(a_b.is_some());
    ...
    fn test_r3() {
        ...
        let tgt = c_2.borrow().r3_referent(&store);
        assert!(tgt.len() == 1);
        assert_eq!(tgt_1, tgt[0]);

        // Test in the many direction
        let c = tgt_0.borrow().r3_c(&store);
        assert!(c.len() == 0);

        let c = tgt_1.borrow().r3_c(&store);
        assert!(c.len() == 2);

        // Result contains c_1 and c_2, but in any order.
        let a_c = c.iter().find(|c| c.borrow().id == c_1.borrow().id);
        assert!(a_c.is_some());
        assert_eq!(Some(&c_1), a_c);

        let a_c = c.iter().find(|c| c.borrow().id == c_2.borrow().id);
        assert!(a_c.is_some());
    ...
    fn test_r4() {
        ...
        // Test in the one direction.
        let tgt = d_0.borrow().r4_referent(&store);
        assert!(tgt.len() == 0);

        let tgt = d_1.borrow().r4_referent(&store);
        assert!(tgt.len() == 1);
        assert_eq!(tgt_1, tgt[0]);

        let tgt = d_2.borrow().r4_referent(&store);
        assert!(tgt.len() == 1);
        assert_eq!(tgt_1, tgt[0]);

        // Test in the many direction
        let d = tgt_0.borrow().r4_d(&store);
        assert!(d.len() == 0);

        let d = tgt_1.borrow().r4_d(&store);
        assert!(d.len() == 2);

        // Result contains d_1 and d_2, but in any order.
        let a_d = d.iter().find(|d| d.borrow().id == d_1.borrow().id);
        assert!(a_d.is_some());
        assert_eq!(Some(&d_1), a_d);

        let a_d = d.iter().find(|d| d.borrow().id == d_2.borrow().id);
        assert!(a_d.is_some());
