...
impl AssociativeReferent {
    ...
    pub fn new(obj_id: &Object, store: &mut SarzakDomainStore) -> AssociativeReferent {
    ...
    pub fn r25_object<'a>(&'a self, store: &'a SarzakDomainStore) -> Vec<&Object> {
    ...
    pub fn r22_associative<'a>(&'a self, store: &'a SarzakDomainStore) -> Vec<&Associative> {
        vec![
            store
                .iter_associative()
    ...
    pub fn r23_associative<'a>(&'a self, store: &'a SarzakDomainStore) -> Vec<&Associative> {
        vec![
            store
                .iter_associative()
