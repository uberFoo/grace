...
impl AssociativeReferent {
    ...
    pub fn object_r25<'a>(&'a self, store: &'a SarzakStore) -> Vec<&Object> {
    ...
    pub fn associative_r23<'a>(&'a self, store: &'a SarzakStore) -> Vec<&Associative> {
        vec![
            store
                .iter_associative()
    ...
    pub fn associative_r22<'a>(&'a self, store: &'a SarzakStore) -> Vec<&Associative> {
        vec![
            store
                .iter_associative()
