...
pub(crate) fn init_woog(
    ...
    for obj in objects {
        ...
        if config.is_external(&obj.id) {
            ...
            inter_external(obj, &ext, module, config, domain, &mut woog);
        } else if inner_object_is_struct(obj, config, domain) {
            log::debug!("Populating woog for struct: {}", obj.name);
            inter_method_new(obj, module, config, domain, &mut woog);
        }
    }
...
fn inter_external(
    ...
    let field = Field::new("ext_value".to_owned(), &ty, woog);
    let field = StructureField::new(None, &field, &structure, woog);
    fields.push(field);

    let param = Parameter::new(Uuid::new_v4(), Some(&function), None, woog);
    let var = Variable::new_parameter("ext_value".to_owned(), &table, &param, woog);
